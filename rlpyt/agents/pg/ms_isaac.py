...
class MS_IsaacMixin:
    ...
    def data_parallel(self, *args, **kwargs):
        raise ValueError("Does not implement data parallel.")


class IsaacFfAgent(MS_IsaacMixin, GaussianPgAgent):

    def __init__(self, ModelCls=MujocoFfModel, **kwargs):
        super().__init__(ModelCls=ModelCls, **kwargs)


class IsaacLstmAgent(MS_IsaacMixin, RecurrentGaussianPgAgent):
